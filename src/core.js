      ...
      if (key.match(':')) {
        ...
        while (--i) {
          split[i].replace(regexPseudoReplace, function (match, name, value) {
            var pseudo = target && typeof(target[name]) != 'undefined' ? target[name] : xtag.pseudos[name];
            if (!pseudo) throw "pseudo not found: " + name + " " + split;
            value = (value === '' || typeof value == 'undefined') ? null : value;
            pseudo = pseudos[i] = Object.create(pseudo);
            pseudo.key = key;
            pseudo.name = name;
            pseudo.value = value;
